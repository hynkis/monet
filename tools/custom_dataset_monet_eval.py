...
class customDataset(Dataset):
    ...
    def __init__(self, dataset_dir, transforms=None, window=0, roi_aug_prob=0.5):
        ...
        file_list = natsort.natsorted(os.listdir(self.dataset_dir_csv))        
        print("file_list :")
        print(file_list)
    ...
    def load_image_rgb(self, path=None, normalize=False, resize=True, resize_size=None):
        ...
        raw_image = Image.open(path) # RGB image # (W,H)
        if resize:
            # print("[bev] raw_image size :", raw_image.size) #(W,H)
            # (60, 60) (W,H)(pil) -> (64, 64) (W,H)(pil) -> (64, 64,3) (H,W,C)(np) -> (3, 64, 64) (C,H,W)(np)
            # : channel (BGR)
            raw_image = np.transpose(raw_image.resize(resize_size), (2,0,1)) # (H,W,C) to (C,H,W)
            # print("[bev] trans raw_image shape :", raw_image.shape, raw_image.size)
            # print("[bev] sum of channel 0 :", np.sum(raw_image[0,:,:]))
            # print("[bev] sum of channel 1 :", np.sum(raw_image[1,:,:]))
            # print("[bev] sum of channel 2 :", np.sum(raw_image[2,:,:]))
    ...
    def load_image_gray(self, path=None, normalize=False):
        # can use any other library too like OpenCV as long as you are consistent with it
        raw_image = Image.open(path) # grayscale image # (W,H)

        # PIL crop: (leftup_x, leftup_y, rightdown_x, rightdown_y)
        # print("[img] raw_image size :", raw_image.size)
        img_crop_area = (self.CROP_POINT[0], self.CROP_POINT[1], self.CROP_POINT[0] + self.CROP_SHAPE[0], self.CROP_POINT[1]+self.CROP_SHAPE[1])
        raw_image = raw_image.crop(img_crop_area) # (640, 480) -> (440, 240)
        # print("[img] crop_image size :", raw_image.size)

        raw_image_tmp_np = np.array(raw_image)
        # print("[img] raw_image_tmp_np shape :", raw_image_tmp_np.shape)


        raw_image = np.array(raw_image.resize(self.RESIZE_SHAPE)) # (440, 240) -> (224, 224) (W,H)(pil) -> (224,224) (H,W)(np)
        # print("[img] resize raw_image shape :", raw_image.shape, raw_image.size)
        raw_image = np.expand_dims(raw_image, axis=0) # (224,224) to (1,224,224): (C,H,W)
        # print("[img] expend raw_image shape :", raw_image.shape, raw_image.size)
        imx_t = np.array(raw_image, dtype=np.float32) / 255.
        # print("[img] np raw_image shape :", raw_image.shape, raw_image.size)
        if normalize:
            imx_t -= 0.5
            imx_t *= 2.
    ...
    def load_image_gray_w_randAugment(self, path, state, label, normalize=False):
        ...
        # (440, 240) -> (224, 224) (W,H)(pil) -> (224,224) (H,W)(np) -> (1,224,224) (C,H,W)(np)
