...
class customDataset(Dataset):
    ...
    def __init__(self, dataset_dir, transforms=None, window=0):
        ...
        self.DEL_STEER_MAX = 200
        self.DEL_THROTTLE_MAX = 200
        self.MAX_ROUTE_DIST = 10
        self.MIN_ROUTE_DIST = 2
        ...
        target_wpt_x_t_list = self.state_array_raw['target_wpt_x_t'].to_numpy()
        target_wpt_y_t_list = self.state_array_raw['target_wpt_y_t'].to_numpy()
        target_wpt_x_t1_list = self.state_array_raw['target_wpt_x_t1'].to_numpy()
        target_wpt_y_t1_list = self.state_array_raw['target_wpt_y_t1'].to_numpy()
        route_dist_t, route_heading_t = \
                self.calc_route_info(pose_x_list, pose_y_list, pose_yaw_list, target_wpt_x_t_list, target_wpt_y_t_list)
        route_dist_t1, route_heading_t1 = \
                self.calc_route_info(pose_x_list, pose_y_list, pose_yaw_list, target_wpt_x_t1_list, target_wpt_y_t1_list)

        self.state_array = self.dataset_csv[['imu_accel_x', 'imu_accel_y', 'imu_yaw_rate']]
        # self.state_array['route_dist_t'] = route_dist_t
        # self.state_array['route_heading_t'] = route_heading_t
        # self.state_array['route_dist_t1'] = route_dist_t1
        # self.state_array['route_heading_t1'] = route_heading_t1

        route_progress_t  = self.map_route_progress(route_dist_t)
        route_progress_t1 = self.map_route_progress(route_dist_t1)
        self.state_array['route_progress_t']  = route_progress_t
        self.state_array['route_heading_t']   = route_heading_t
        self.state_array['route_progress_t1'] = route_progress_t1
        self.state_array['route_heading_t1']  = route_heading_t1
        ...
        norm_steer    = (self.label_array.loc[:,'steer_t'] - self.STEER_NEUTRAL) / self.DEL_STEER_MAX
        norm_throttle = (self.label_array.loc[:,'throttle_t'] - self.THROTTLE_NEUTRAL) / self.DEL_THROTTLE_MAX
    ...
    def __getitem__(self, index):
        ...
        if self.transforms:
            rand_noise_type = random.choice(['motion_blur'])  
    ...
    def load_image_gray_w_randAugment(self, path, state, label, normalize=False):
        ...
        if state[3] < 0.5:
            ...
            label[0] += shifting_size * 1/400. # 50/400 * [-2,-1,0,+1,+2]
        else:
            shifting_size = 0
    ...
    def map_route_progress(self, route_dist):
        # route_dist: ndarray
        route_dist = np.minimum(self.MAX_ROUTE_DIST, np.maximum(self.MIN_ROUTE_DIST, route_dist))
        # route_dist -> route_progress = 
        route_progress = -1./(self.MAX_ROUTE_DIST - self.MIN_ROUTE_DIST) * (route_dist - self.MAX_ROUTE_DIST)
        ...
        for i, (data, idx) in enumerate(dataloader):
            ...
            for j in range(BATCH_SIZE):
                ...
                print("-------------- route_progress_t : %f, route_head_t : %f, route_progress_t1 : %f, route_head_t1 : %f" \
                        %(data["state"][j,3], data["state"][j,4], data["state"][j,5], data["state"][j,6]))
                print("-------------- steer : %f, throttle : %f" %(data["label"][j,0], data["label"][j,1]))
                dataset.custom_torch_imshow_rgb(data["image"][j], denormalize=True)
                dataset.custom_torch_imshow_rgb(data["bev"][j], denormalize=True)
